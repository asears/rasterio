...
def test_err(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["($ 0.1 (read 1))", "tests/data/shade.tif", outfile],
...
def test_multiband_calc(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["(+ 125 (* 0.1 (read 1)))", "tests/data/shade.tif", outfile],
        catch_exceptions=False,
    )
...
def test_singleband_calc_byindex(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["(+ 125 (* 0.1 (read 1 1)))", "tests/data/shade.tif", outfile],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    with rasterio.open(outfile) as src:
...
def test_copy_rgb(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["(read 1)", "tests/data/RGB.byte.tif", outfile],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    with rasterio.open(outfile) as src:
...
def test_fillnodata_map(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"]
        + ["(asarray (map fillnodata (read 1)))", "tests/data/RGB.byte.tif", outfile],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    with rasterio.open(outfile) as src:
...
def test_sieve_band(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["(sieve (band 1 1) 42)", "tests/data/shade.tif", outfile],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    with rasterio.open(outfile) as src:
...
def test_sieve_read(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["calc"] + ["(sieve (read 1 1 'uint8') 42)", "tests/data/shade.tif", outfile],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    with rasterio.open(outfile) as src:
