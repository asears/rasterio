    ...
    with rasterio.open("tests/data/RGB.byte.tif") as src:
        try:
            show_hist(
                ...
                alpha=0.3,
                histtype="stepfilled",
                title="World Histogram overlaid",
            )
            fig = plt.gcf()
            plt.close(fig)
        except ImportError:
    ...
    with rasterio.open("tests/data/RGB.byte.tif") as src:
        try:
            show(
                ...
                colors=["white", "red"],
                linewidths=4,
                contour_label_kws=dict(
                    fontsize=18, fmt="%1.0f", inline_spacing=15, use_clabeltext=True
                ),
            )
            fig = plt.gcf()
            plt.close(fig)
...
def test_plotting_extent():
    ...
    with rasterio.open("tests/data/RGB.byte.tif") as src:
        ...
        assert (
            plotting_extent(reshape_as_image(src.read()), transform=src.transform)
            == expected
        )
        assert plotting_extent(src.read(1), transform=src.transform) == expected
