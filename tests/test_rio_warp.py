...
def test_dst_crs_error(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "{foo: bar}"]
    )
...
def test_dst_crs_error_2(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", '{"proj": "foobar"}']
    )
    assert result.exit_code == 2
...
def test_dst_crs_error_epsg(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "EPSG:"]
    )
    assert result.exit_code == 2
...
def test_dst_crs_error_epsg_2(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "EPSG:0"]
    )
    assert result.exit_code == 2
...
def test_dst_nodata_float_no_src_nodata_err(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-nodata", "0.0"]
    )
    assert result.exit_code == 2
...
def test_src_nodata_int_ok(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--src-nodata", "1"]
    )
...
def test_dst_nodata_int_ok(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-nodata", "255"]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as src:
...
def test_src_nodata_float_ok(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--src-nodata", "1.5"]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as src:
...
def test_warp_no_reproject_dimensions(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dimensions", "100", "100"]
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
    ...
    with rasterio.open(srcname) as src:
        with rasterio.open(outputname) as output:
            ...
            assert np.allclose(
                [97.839396, 97.839396], [output.transform.a, -output.transform.e]
            )
...
def test_warp_no_reproject_res(runner, tmpdir):
    srcname = "tests/data/shade.tif"
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(main_group, ["warp", srcname, outputname, "--res", 30])
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_no_reproject_bounds(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--bounds"] + out_bounds
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_no_reproject_bounds_res(runner, tmpdir):
    ...
    out_bounds = [-11850000, 4810000, -11849000, 4812000]
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--res", 30, "--bounds"] + out_bounds
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_no_reproject_src_bounds_dimensions(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group,
        ["warp", srcname, outputname, "--dimensions", 9, 14, "--src-bounds"]
        + out_bounds,
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
    ...
    with rasterio.open(srcname) as src:
        with rasterio.open(outputname) as output:
            ...
            assert np.allclose(output.bounds, out_bounds)
            assert np.allclose(
                [111.111111, 142.857142], [output.transform.a, -output.transform.e]
            )
...
def test_warp_reproject_dst_crs(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "EPSG:4326"]
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_reproject_dst_crs_proj4(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", proj4]
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_reproject_dimensions(runner, tmpdir):
    ...
    with rasterio.open(outputname) as output:
        ...
        assert output.height == 100
        assert np.allclose(
            [0.0008789062498762235, 0.0006771676143921468],
            [output.transform.a, -output.transform.e],
        )
...
def test_warp_reproject_dimensions_invalid_params(runner, tmpdir):
    srcname = "tests/data/shade.tif"
    outputname = str(tmpdir.join("test.tif"))
    bad_params = [["--bounds", "0", "0", "10", "10"], ["--res", "10"]]
...
def test_warp_reproject_multi_bounds_fail(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group,
        ["warp", srcname, outputname, "--dst-crs", "EPSG:4326", "--src-bounds"]
        + out_bounds
        + ["--bounds"]
        + out_bounds,
    )
...
def test_warp_reproject_src_bounds_res(runner, tmpdir):
    ...
    with rasterio.open(outputname) as output:
        assert output.crs == {"init": "epsg:4326"}
        assert np.allclose(output.bounds[:], [-106.45036, 39.6138, -106.44136, 39.6278])
        assert np.allclose([0.001, 0.001], [output.transform.a, -output.transform.e])
        assert output.width == 9
        assert output.height == 14
...
def test_warp_reproject_dst_bounds(runner, tmpdir):
    ...
    with rasterio.open(outputname) as output:
        assert output.crs == {"init": "epsg:4326"}
        assert np.allclose(output.bounds[0::3], [-106.45036, 39.6278])
        assert np.allclose([0.001, 0.001], [output.transform.a, -output.transform.e])
        ...
        assert np.allclose(
            [output.bounds[2] - 0.001, output.bounds[1] + 0.001], [-106.44136, 39.6138]
        )
...
def test_warp_reproject_like(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--like", likename]
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_reproject_like_invalid_params(runner, tmpdir):
    ...
    for param in bad_params:
        result = runner.invoke(
            warp.warp, [srcname, outputname, "--like", srcname] + param
        )
...
def test_warp_reproject_nolostdata(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "EPSG:3857"]
    )
    assert result.exit_code == 0
    assert os.path.exists(outputname)
...
def test_warp_dst_crs_empty_string(runner, tmpdir):
    ...
    result = runner.invoke(main_group, ["warp", infile, outfile, "--dst-crs", ""])
...
def test_warp_reproject_check_invert_true(runner, tmpdir):
    ...
    # default True
    runner.invoke(main_group, ["warp", srcname, outputname, "--dst-crs", "EPSG:3759"])

    # explicit True
    runner.invoke(
        main_group,
        ["warp", srcname, output2name, "--check-invert-proj", "--dst-crs", "EPSG:3759"],
    )
...
def test_warp_vrt_gcps(runner, tmpdir):
    ...
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["warp", srcname, outputname, "--dst-crs", "epsg:32618"]
    )
    assert result.exit_code == 0
...
@pytest.mark.skipif(
    GDALVersion.runtime().at_least("2.0"), reason="Test only applicable to GDAL < 2.0"
)
@pytest.mark.parametrize("method", GDAL2_RESAMPLING)
def test_warp_resampling_not_yet_supported(runner, path_rgb_byte_tif, tmpdir, method):
