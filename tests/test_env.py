...
def test_ensured_env_no_credentializing(gdalenv):
    ...
    with rasterio.Env(aws_access_key_id="foo", aws_secret_access_key="bar"):
...
def test_env_discovery(key, val):
    ...
    assert (
        rasterio.env.local._discovered_options is None
...
def test_require_gdal_version_param_values_version_too_high():
    ...
    @require_gdal_version(version, param="foo", values=["bar"], is_max_version=True)
    def a(foo=None):
        return foo
...
def test_dummy_session_with_boto3_expired_credentials(monkeypatch, caplog):
    ...
    with monkeypatch.context() as mpctx:
        ...
        mpctx.setenv(
            "AWS_SESSION_TOKEN",
            "FQoGZXIvYXdzEH0aDIyhzy8S3gql/UawbyKrAa3R03j4JBTNfRHmfBAaGQ366PsaWfO+cHtrRS",
        )
