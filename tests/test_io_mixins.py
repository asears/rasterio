...
def test_windows_mixin():
    ...
    assert_window_almost_equals(
        src.window(*src.bounds), Window(0, 0, src.width, src.height)
    )

    assert src.window_bounds(Window(0, 0, src.width, src.height)) == src.bounds

    assert src.window_transform(Window(0, 0, src.width, src.height)) == src.transform
...
def test_window_transform_method():
    with rasterio.open("tests/data/RGB.byte.tif") as src:
        ...
        assert (
            src.window_transform(Window(1, 1, None, None)).c
