...
def test_env(runner):
    result = runner.invoke(main_group, ["env", "--formats"])
...
def test_info_err(runner):
    ...
    result = runner.invoke(main_group, ["info", "tests"])
    assert result.exit_code != 0
...
def test_info(runner):
    ...
    result = runner.invoke(main_group, ["info", "tests/data/float.tif"])
    assert result.exit_code == 0
    info = json.loads(result.output)
    assert info["count"] == 1
...
def test_info_verbose(runner):
    result = runner.invoke(main_group, ["-v", "info", "tests/data/RGB.byte.tif"])
    assert result.exit_code == 0


def test_info_quiet(runner):
    result = runner.invoke(main_group, ["-q", "info", "tests/data/RGB.byte.tif"])
    assert result.exit_code == 0


def test_info_gcps(runner):
    result = runner.invoke(main_group, ["info", "tests/data/white-gemini-iv.vrt"])
    assert result.exit_code == 0


def test_info_count(runner):
    result = runner.invoke(main_group, ["info", "tests/data/RGB.byte.tif", "--count"])
    assert result.exit_code == 0
    assert result.output == "3\n"


def test_info_nodatavals(runner):
    result = runner.invoke(main_group, ["info", "tests/data/RGB.byte.tif", "--bounds"])
    assert result.exit_code == 0
    assert result.output == "101985.0 2611485.0 339315.0 2826915.0\n"


def test_info_tags(runner):
    result = runner.invoke(main_group, ["info", "tests/data/RGB.byte.tif", "--tags"])
    assert result.exit_code == 0
    assert result.output == '{"AREA_OR_POINT": "Area"}\n'


def test_info_res(runner):
    result = runner.invoke(main_group, ["info", "tests/data/RGB.byte.tif", "--res"])
    assert result.exit_code == 0
    assert result.output.startswith("300.037")


def test_info_lnglat(runner):
    result = runner.invoke(main_group, ["info", "tests/data/RGB.byte.tif", "--lnglat"])
    assert result.exit_code == 0
...
def test_info_stats_only(runner):
    result = runner.invoke(
        main_group, ["info", "tests/data/RGB.byte.tif", "--stats", "--bidx", "2"]
    )
    assert result.exit_code == 0
...
def test_transform_err(runner):
    result = runner.invoke(main_group, ["transform"], "[-78.0]")
...
def test_transform_point_dst_file(runner):
    result = runner.invoke(
        main_group,
        ["transform", "--dst-crs", "tests/data/RGB.byte.tif", "--precision", "2"],
        "[-78.0, 23.0]",
    )
    assert result.exit_code == 0
    assert result.output.strip() == "[192457.13, 2546667.68]"
...
def test_transform_point_src_file(runner):
    result = runner.invoke(
        main_group,
        ["transform", "--src-crs", "tests/data/RGB.byte.tif", "--precision", "2"],
...
def test_transform_point_2(runner):
    result = runner.invoke(
        main_group,
        ["transform", "[-78.0, 23.0]", "--dst-crs", "EPSG:32618", "--precision", "2"],
    )
    assert result.exit_code == 0
    assert result.output.strip() == "[192457.13, 2546667.68]"
...
def test_bounds_defaults(runner):
    result = runner.invoke(main_group, ["bounds", "tests/data/RGB.byte.tif"])
    assert result.exit_code == 0
    assert "Feature" in result.output


def test_bounds_err(runner):
    result = runner.invoke(main_group, ["bounds", "tests"])
    assert result.exit_code == 1


def test_bounds_feature(runner):
    result = runner.invoke(
        main_group, ["bounds", "tests/data/RGB.byte.tif", "--feature"]
    )
    assert result.exit_code == 0
...
def test_bounds_obj_bbox(runner):
    result = runner.invoke(
        main_group, ["bounds", "tests/data/RGB.byte.tif", "--bbox", "--precision", "2"]
    )
    assert result.exit_code == 0
...
def test_bounds_seq(runner):
    result = runner.invoke(
        main_group,
        ["bounds", "tests/data/RGB.byte.tif", "tests/data/RGB.byte.tif", "--sequence"],
    )
    assert result.exit_code == 0
...
def test_info_checksums_only(runner):
    result = runner.invoke(
        main_group, ["info", "tests/data/RGB.byte.tif", "--checksum", "--bidx", "2"]
    )
    assert result.exit_code == 0
