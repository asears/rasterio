...
def test_mask(runner, tmpdir, basic_feature, basic_image_2x2, pixelated_image_file):
    ...
    with rasterio.open(output) as out:
        assert np.array_equal(basic_image_2x2, out.read(1, masked=True).filled(0))


def test_mask_all_touched(
    runner, tmpdir, basic_feature, basic_image, pixelated_image_file
):
    ...
    with rasterio.open(output) as out:
        assert np.array_equal(basic_image, out.read(1, masked=True).filled(0))


def test_mask_invert(
    runner, tmpdir, basic_feature, pixelated_image, pixelated_image_file
):
...
def test_mask_featurecollection(
    runner, tmpdir, basic_featurecollection, basic_image_2x2, pixelated_image_file
):
...
def test_mask_out_of_bounds(runner, tmpdir, basic_feature, pixelated_image_file):
    """
...
def test_mask_no_geojson(runner, tmpdir, pixelated_image, pixelated_image_file):
    ...
    result = runner.invoke(main_group, ["mask", pixelated_image_file, output])
    assert result.exit_code == 0
    assert os.path.exists(output)

    with rasterio.open(output) as out:
        assert np.array_equal(pixelated_image, out.read(1, masked=True).filled(0))
...
def test_mask_crop_inverted_y(runner, tmpdir, basic_feature, pixelated_image_file):
    ...
    result = runner.invoke(
        main_group,
        ["mask", pixelated_image_file, output, "--crop", "--geojson-mask", "-"],
        input=json.dumps(basic_feature),
    )
...
def test_mask_crop_out_of_bounds(runner, tmpdir, basic_feature, pixelated_image_file):
    """
