...
def test_window_no_exception():
    with rasterio.open("tests/data/RGB.byte.tif") as src:
        ...
        assert_window_almost_equals(
            src.window(left, bottom, right, top),
            windows.Window(
                -1000 / src.res[0], 0, src.width + 1000 / src.res[0], src.height
            ),
        )
...
def test_window_union():
    assert windows.union(
        windows.Window(0, 3, 3, 6), windows.Window(2, 1, 2, 4)
...
def test_window_intersection_disjunct():
    with pytest.raises(WindowError):
        windows.intersection(
            ((0, 6), (3, 6)), ((100, 200), (0, 12)), ((7, 12), (7, 12))
        )
...
def test_windows_intersect():
    ...
    assert (
        windows.intersect(((0, 6), (3, 6)), ((2, 4), (1, 5)), ((3, 6), (0, 6))) is True
    )
...
def test_windows_intersect_disjunct():
    assert windows.intersect(((0, 6), (3, 6)), ((10, 20), (0, 6))) is False

    # polygons touch at point
    assert windows.intersect(((0, 2), (1, 3)), ((2, 4), (3, 5))) is False

    # polygons touch at point, rev order
    assert windows.intersect(((2, 4), (3, 5)), ((0, 2), (1, 3))) is False

    # polygons touch at line
    assert windows.intersect(((0, 6), (3, 6)), ((6, 10), (1, 5))) is False

    assert (
        windows.intersect(((0, 6), (3, 6)), ((2, 4), (1, 5)), ((5, 6), (0, 6))) is False
    )

    assert (
        windows.intersect(((0, 6), (3, 6)), ((2, 4), (1, 3)), ((3, 6), (4, 6))) is False
    )


def test_iter_args_winfuncs():
    wins = [((0, 6), (3, 6)), ((2, 4), (1, 5))]
