...
def test_clip_like(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group,
        ["clip", "tests/data/shade.tif", output, "--like", "tests/data/shade.tif"],
    )
    assert result.exit_code == 0
    assert os.path.exists(output)
...
def test_clip_missing_params(runner, tmpdir):
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(main_group, ["clip", "tests/data/shade.tif", output])
...
def test_clip_like_disjunct(runner, tmpdir):
    ...
    result = runner.invoke(
        main_group,
        ["clip", "tests/data/shade.tif", output, "--like", "tests/data/RGB.byte.tif"],
    )
    assert result.exit_code == 2
...
def test_format_short(tmpdir, runner):
    outputname = str(tmpdir.join("test.jpg"))
    result = runner.invoke(
        main_group, ["convert", "tests/data/RGB.byte.tif", outputname, "-f", "JPEG"]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as src:
        assert src.driver == "JPEG"
...
def test_autodetect_format(tmpdir, runner, extension, driver):
    ...
    result = runner.invoke(
        main_group, ["convert", "tests/data/RGB.byte.tif", outputname]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as src:
...
def test_rgb(tmpdir, runner):
    outputname = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["convert", "tests/data/RGB.byte.tif", outputname, "--rgb"]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as src:
