...
def test_stack(tmpdir, runner):
    ...
    result = runner.invoke(main_group, ["stack", "tests/data/RGB.byte.tif", outputname])
...
def test_stack_list(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group, ["stack", "tests/data/RGB.byte.tif", "--bidx", "1,2,3", outputname]
    )
    assert result.exit_code == 0
    with rasterio.open(outputname) as out:
        assert out.count == 3
...
def test_format_jpeg(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group, ["stack", "tests/data/RGB.byte.tif", outputname, "--format", "JPEG"]
    )
    assert result.exit_code == 0
...
def test_error(tmpdir, runner):
    ...
    result = runner.invoke(
        main_group,
        ["stack", "tests/data/RGB.byte.tif", outputname, "--driver", "BOGUS"],
    )
