...
@requires_gdal22(
    reason="Some versions prior to 2.2.2 segfault on a Mac OSX " "Homebrew GDAL"
)
def test_ycbcr_interp(tmpdir):
    ...
    with rasterio.open(tiffname, "w", **meta) as dst:
        assert dst.colorinterp == (ColorInterp.red, ColorInterp.green, ColorInterp.blue)
...
def test_set_colorinterp(path_rgba_byte_tif, tmpdir, dtype):
    ...
    dst_ci = (ColorInterp.alpha, ColorInterp.blue, ColorInterp.green, ColorInterp.red)
...
def test_set_colorinterp_all(path_4band_no_colorinterp, ci):
    ...
    if ci.value == 1:
        pytest.xfail(
            "Setting colorinterp to gray fails with GDAL 2.3, see https://github.com/mapbox/rasterio/issues/1234"
        )
