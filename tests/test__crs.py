...
@pytest.mark.parametrize(
    "proj,expected", [({"init": "epsg:4326"}, False), ({"init": "epsg:3857"}, True)]
)
...
@pytest.mark.parametrize(
    "proj_string", ["+init=epsg:4326", "+proj=longlat +datum=WGS84 +no_defs"]
)
...
def test_to_esri_wkt_fix_datum():
    """Morph to Esri form"""
    assert 'DATUM["D_North_American_1983"' in _CRS.from_dict(init="epsg:26913").to_wkt(
