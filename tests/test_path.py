...
def test_vsi_path_scheme():
    ...
    assert (
        vsi_path(ParsedPath("/foo.tif", "tests/data/files.zip", "zip"))
        == "/vsizip/tests/data/files.zip/foo.tif"
    )
...
def test_path_as_vsi_scheme():
    """Correctly make a vsi path"""
    assert (
        ParsedPath("/foo.tif", "tests/data/files.zip", "zip").as_vsi()
        == "/vsizip/tests/data/files.zip/foo.tif"
    )
...
def test_vsi_path_curl():
    ...
    assert (
        vsi_path(ParsedPath("example.com/foo.tif", None, "https"))
...
def test_read_vfs_file():
    with rasterio.open("file://tests/data/RGB.byte.tif") as src:
...
def test_parse_path_win():
    pathlib = pytest.importorskip("pathlib")
    assert isinstance(
        parse_path(pathlib.PureWindowsPath(r"C:\foo\bar.tif")), ParsedPath
    )
