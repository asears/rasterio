...
class ReaderContextTest(unittest.TestCase):
    def test_context(self):
        ...
        self.assertEqual(
            repr(s), "<closed DatasetReader name='tests/data/RGB.byte.tif' " "mode='r'>"
        )
    ...
    def test_read_out_dtype_fail(self):
        with rasterio.open("tests/data/RGB.byte.tif") as s:
            ...
            try:
                ...
                assert (
                    "the array's dtype 'float32' does not match the " "file's dtype"
                ) in str(e)
            except:
    ...
    def test_read_indexes(self):
        with rasterio.open("tests/data/RGB.byte.tif") as s:
            ...
            self.assertEqual(a.ndim, 3)
            self.assertEqual(a.shape, (3, 718, 791))
            self.assertEqual(a.sum((1, 2)).tolist(), [17008452, 25282412, 27325233])
    ...
    def test_read_window(self):
        with rasterio.open("tests/data/RGB.byte.tif") as s:
            ...
            self.assertEqual(
                [md5(x.tobytes()).hexdigest() for x in a],
                [
