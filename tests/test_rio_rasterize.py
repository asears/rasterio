...
def test_rasterize(tmpdir, runner, basic_feature):
    ...
    result = runner.invoke(
        main_group,
        ["rasterize", output, "--dimensions", DEFAULT_SHAPE[0], DEFAULT_SHAPE[1]],
...
def test_rasterize_like_raster(
    tmpdir, runner, basic_feature, basic_image_2x2, pixelated_image_file
):
...
def test_rasterize_like_raster_src_crs_mismatch(
    tmpdir, runner, basic_feature, pixelated_image_file
):
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group,
...
def test_rasterize_featurecollection(
    tmpdir, runner, basic_feature, pixelated_image_file
):
    output = str(tmpdir.join("test.tif"))
    collection = {"type": "FeatureCollection", "features": [basic_feature]}
    result = runner.invoke(
        main_group,
        ["rasterize", output, "--like", pixelated_image_file],
...
def test_rasterize_src_crs_mismatch(
    tmpdir, runner, basic_feature, pixelated_image_file
):
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(
...
def test_rasterize_like_raster_outside_bounds(
    tmpdir, runner, basic_feature, pixelated_image_file
):
    """
...
def test_rasterize_invalid_stdin(tmpdir, runner):
    ...
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(main_group, ["rasterize", output], input="BOGUS")
...
def test_rasterize_invalid_geojson(tmpdir, runner):
    ...
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(main_group, ["rasterize", output], input='{"A": "B"}')
...
def test_rasterize_missing_parameters(tmpdir, runner, basic_feature):
    ...
    output = str(tmpdir.join("test.tif"))
    result = runner.invoke(
        main_group, ["rasterize", "-o", output], input=json.dumps(basic_feature)
    )
