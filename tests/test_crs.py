...
def test_write_3857(tmpdir):
    ...
    subprocess.call(
        ["gdalwarp", "-t_srs", "epsg:3857", "tests/data/RGB.byte.tif", src_path]
    )
...
@pytest.mark.parametrize(
    "proj,expected", [({"init": "epsg:4326"}, True), ({"init": "epsg:3857"}, False)]
)
...
@requires_gdal22(reason="GDAL bug resolved in 2.2+ allowed invalid CRS to be created")
...
def test_to_esri_wkt_fix_datum():
    """Morph to Esri form"""
    assert 'DATUM["D_North_American_1983"' in CRS(init="epsg:26913").to_wkt(
        morph_to_esri_dialect=True
    )
...
def test_linear_units_factor():
    ...
    assert CRS.from_epsg(2261).linear_units_factor[1] == pytest.approx(
        0.3048006096012192
    )
    with pytest.raises(CRSError):
