...
def check_features_block_windows(features, src, bidx):
    ...
    for feat, (block, window) in iterator:
        ...
        out.append(np.array_equal(json.loads(feat["properties"]["block"]), block))
...
def test_windows(runner, path_rgb_byte_tif):
    result = runner.invoke(main_group, ["blocks", path_rgb_byte_tif])
...
def test_windows_precision(runner, path_rgb_byte_tif):
    result = runner.invoke(main_group, ["blocks", path_rgb_byte_tif, "--precision", 1])
    assert result.exit_code == 0
...
def test_windows_indent(runner, path_rgb_byte_tif):
    result = runner.invoke(main_group, ["blocks", path_rgb_byte_tif, "--indent", 4])
    assert result.exit_code == 0
...
def test_windows_exception(runner, path_rgb_byte_tif):
    ...
    not have 4 bands.
    """
    result = runner.invoke(main_group, ["blocks", path_rgb_byte_tif, "--bidx", 4])
    assert result.exit_code == 2
    assert "Not a valid band index" in result.output


def test_windows_projected(runner, path_rgb_byte_tif):
    result = runner.invoke(main_group, ["blocks", path_rgb_byte_tif, "--projected"])
    assert result.exit_code == 0
