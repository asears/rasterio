...
def test_copy_strict_failure(tmpdir, path_float_tif):
    ...
    with pytest.raises(CPLE_NotSupportedError):
        rasterio.shutil.copy(path_float_tif, outfile, strict=True, driver="JPEG")
...
def test_copy_strict_silent_failure(tmpdir, path_float_tif):
    ...
    rasterio.shutil.copy(path_float_tif, outfile, strict=False, driver="JPEG")
...
def test_copyfiles(data, tmpdir):
    ...
    with rasterio.open(infile) as expected, rasterio.open(outfile) as actual:
