...
def mask(
    ...
    if geojson_mask is None:
        click.echo("No GeoJSON provided, INPUT will be copied to OUTPUT", err=True)
    ...
    with ctx.obj["env"]:
        try:
            ...
            raise click.BadParameter(
                "GeoJSON could not be read from " "--geojson-mask or stdin",
        ...
        if "features" in geojson:
            ...
            raise click.BadParameter("Invalid GeoJSON", param=input, param_hint="input")

        with rasterio.open(input) as src:
            try:
                out_image, out_transform = mask_tool(
                    src, geometries, crop=crop, invert=invert, all_touched=all_touched
                )
