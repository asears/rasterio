...
@click.option(
    ...
    type=float,
    default=1,
    help="Default value for rasterized pixels",
)
@click.option(
    "--fill",
    type=float,
...
def rasterize(
    ...
    try:
        src_crs = (
            CRS.from_string(src_crs) if has_src_crs else CRS.from_string("EPSG:4326")
        )
    except CRSError:
        raise bad_param
        ...
        if rasterio.shutil.exists(output):
            ...
            if like is not None:
                ...
                if has_src_crs and src_crs != template_ds.crs:
                    raise click.BadParameter(
                        "GeoJSON does not match crs of " "--like raster",
                        param="input",
                        param_hint="input",
                    )
